...
class AndroidConnectionWidget(BaseEMGWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.android_ip = "172.20.10.8"  # 기본 IP
...
class MainMenuWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # 환영 메시지와 안드로이드 연결 버튼을 같은 행에 배치
        welcome_row_layout = QHBoxLayout()
        ...
        self.hand_model_btn.setFixedSize(120, 45)
        ...
        welcome_row_layout.addWidget(self.welcome_label, 3)
        welcome_row_layout.addStretch()
        welcome_row_layout.addLayout(buttons_layout)  # 버튼 레이아웃 추가
        
        welcome_layout.addLayout(welcome_row_layout)
...
class ScreenControlWidget(BaseEMGWidget):
    ...
    def init_ui(self):
        """UI 초기화 - 동작1~8 균등 크기, 가위바위보만 가운데 정렬"""
        layout = QVBoxLayout(self)
        layout.setContentsMargins(20, 20, 20, 20)
        ...
        # 세 번째 행: 가위/바위/보 (데이터 수집과 동일한 색상)
        actions_row3 = [
            ("가위", LABEL_SCISSORS, "#F0F8E8"),  # 데이터 수집과 동일한 연한 녹색
        ...
        for row, actions in enumerate([actions_row1, actions_row2, actions_row3]):
            for col, (action_name, label, bg_color) in enumerate(actions):
                # 동작 카드 생성 (데이터 수집과 비슷한 크기)
                action_card_item = NeumorphicCard()
                action_card_item.setFixedHeight(120)  # 데이터 수집과 같은 높이
                
                # 기본 스타일 설정 (라벨 투명성 확실히)
                ...
                # 그리드에 카드 추가 - 수정된 부분
                if row <= 1:  # 동작 1-8 (0행, 1행) - 기존 방식, 스트레치 영향 없음
        ...
        rps_layout.addStretch()  # 왼쪽 여백
        
        # 가위바위보 카드들 생성
        for col, (action_name, label, bg_color) in enumerate(actions_row3):
            # 가위바위보 카드 생성
            rps_card_item = NeumorphicCard()
            rps_card_item.setFixedHeight(120)
            rps_card_item.setFixedWidth(200)  # 고정 너비로 설정
            
            rps_card_item.setStyleSheet(f"""
                NeumorphicCard {{
                    background-color: {bg_color};
                    border-radius: 12px;
                    border: none;
            ...
            rps_card_layout = QVBoxLayout(rps_card_item)
            rps_card_layout.setContentsMargins(8, 8, 8, 8)
            
            # 동작 이름 레이블
            rps_label = QLabel(action_name)
            rps_label.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 24px; font-weight: bold; background-color: transparent !important; border: none !important;")
            rps_label.setAlignment(Qt.AlignCenter)
            
            # 기능 선택 콤보박스
            rps_combo = QComboBox()
            rps_combo.addItems(self.available_actions[self.current_app])
            rps_combo.setCurrentText(self.action_settings[self.current_app].get(str(label), "없음"))
            rps_combo.currentTextChanged.connect(lambda text, l=label: self.update_action_setting(self.current_app, l, text))
            ...
            if col < 2:  # 가위, 바위 다음에 간격 추가
                rps_layout.addSpacing(10)
            
            # 콤보박스 저장
            self.action_combo_boxes[label] = rps_combo
        
        rps_layout.addStretch()  # 오른쪽 여백
        
        # 액션 레이아웃에 그리드와 가위바위보 레이아웃 추가
        action_layout.addWidget(self.current_app_label)
        action_layout.addLayout(action_grid)  # 동작 1-8
        action_layout.addLayout(rps_layout)   # 가위바위보
    ...
    def select_app(self, app_key):
        ...
        for label, combo in self.action_combo_boxes.items():
            ...
            combo.setCurrentText(self.action_settings[app_key].get(str(label), "없음"))
