...
class MainMenuWidget(QWidget):
    ...
    def on_training_clicked(self):
        ...
        if isinstance(main_window, EMGGameApplication):
            main_window.adjust_window_size(1560, 1200)
        
        # 약간 지연 후 화면 전환 (애니메이션이 보이도록)
        QTimer.singleShot(100, lambda: self.show_data_collection.emit())
    ...
    def on_screen_control_clicked(self):
        ...
        # 약간 지연 후 화면 전환 (애니메이션이 보이도록)
        QTimer.singleShot(100, lambda: self.show_screen_control.emit())
...
class DataCollectionWidget(BaseEMGWidget):
    ...
    <h2 style='color: #5e72e4;'>동작 데이터 수집</h2>
    <p>가위/바위/보와 추가 동작을 수집합니다.</p>
    <p>각 동작별로 최소 5번씩 수집하여 모델 학습에 사용합니다.</p>
    <p>잘못 입력한 경우 '마지막 데이터 삭제' 버튼을 눌러 삭제할 수 있습니다.</p>
    <p>추가 동작은 화면 제어 및 촬영 모드에서 사용 가능합니다.</p>
        """)
        ...
        # 동작 선택 영역 제목
        action_selection_title = QLabel("동작을 클릭하여 선택하세요")
        action_selection_title.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 20px; font-weight: bold;")
        action_selection_title.setAlignment(Qt.AlignCenter)
        
        # 진행 상태를 표시할 그리드 레이아웃 (가로로 배열)
        ...
        # 첫 번째 행: 가위/바위/보
        actions_row1 = [
            ("가위", LABEL_SCISSORS, "#EEF2FF", PRIMARY_COLOR),
        ...
        for row, actions in enumerate([actions_row1, actions_row2, actions_row3]):
            for col, (action_name, label, bg_color, progress_color) in enumerate(actions):
                ...
                action_label.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 16px; font-weight: bold;")
                action_label.setAlignment(Qt.AlignCenter)
                
                # 카운트 레이블
                count_label = QLabel(f"0/{self.target_count}")
                count_label.setAlignment(Qt.AlignCenter)
                count_label.setStyleSheet(f"color: {progress_color}; font-size: 18px; font-weight: bold;")
    ...
    def on_label_selection_changed(self, index):
        """라벨 선택 변경 처리"""
        if index >= 0:
            selected_label = self.label_selection_combo.itemData(index)
            if selected_label:
                self.current_label = selected_label
                self.action_current_label.setText(f"현재 동작: 라벨 {self.current_label}")
                
                # 버튼 상태 업데이트
                self.action_next_btn.setEnabled(self.label_counts[self.current_label] >= 5)
                
                # 선택된 라벨 카드 하이라이트
                for i, card in self.action_cards.items():
                    if i == self.current_label:
                        ...
                        for child in card.children():
                            if isinstance(child, QLabel) and child != self.action_count_labels[i]:
                                child.setStyleSheet("color: white; font-size: 16px; font-weight: bold;")
                    else:
                        card.setStyleSheet(f"""
                        ...
                        for child in card.children():
                            if isinstance(child, QLabel) and child != self.action_count_labels[i]:
                                child.setStyleSheet(f"color: {PRIMARY_COLOR}; font-size: 16px; font-weight: bold;")
                
                self.add_log(f"라벨 {self.current_label} 데이터 수집 준비 완료")
    
    def next_action(self, mode):
        """다음 동작으로 이동 - 통합 버전"""
        # 이전 선택된 카드 스타일 복원
        if hasattr(self, 'current_label') and self.current_label in self.action_cards:
            if self.current_label == LABEL_SCISSORS:
                self.action_cards[self.current_label].setStyleSheet(f"background-color: #EEF2FF; border-radius: 15px;")
            elif self.current_label == LABEL_ROCK:
                self.action_cards[self.current_label].setStyleSheet(f"background-color: #FFFBEB; border-radius: 15px;")
            elif self.current_label == LABEL_PAPER:
                self.action_cards[self.current_label].setStyleSheet(f"background-color: #E8F5E9; border-radius: 15px;")
            elif 1 <= self.current_label <= 8:
                self.action_cards[self.current_label].setStyleSheet(f"background-color: #E6F4FF; border-radius: 15px;")
        
        if mode == "game":
            # 현재 라벨에 따라 다음 라벨 설정
            if self.current_label is None:
                # 첫 동작은 가위로 시작
                next_label = LABEL_SCISSORS
            elif self.current_label == LABEL_SCISSORS and self.label_counts[LABEL_SCISSORS] >= 5:
                next_label = LABEL_ROCK
            elif self.current_label == LABEL_ROCK and self.label_counts[LABEL_ROCK] >= 5:
                next_label = LABEL_PAPER
            elif self.current_label == LABEL_PAPER and self.label_counts[LABEL_PAPER] >= 5:
                # 모든 동작 수집 완료
                self.current_label = None
                self.current_action_label.setText("모든 게임 동작 수집 완료!")
                self.add_log("모든 게임 동작 데이터 수집이 완료되었습니다.")
                return
            else:
                next_label = LABEL_SCISSORS  # 기본값
                    
            # 다음 동작 선택
            self.on_action_card_clicked(next_label)
        else:  # action
            # 현재 라벨에 따라 다음 라벨 설정 (1-8 범위)
            current = self.current_label if 1 <= self.current_label <= 8 else 0
            next_label = current + 1 if current < 8 else 1
    ...
    def check_completion(self):
        ...
        # 동작 제어 모드 완료 확인 (최소 3개의 라벨 각각 5개 이상 있어야 함)
        action_labels_with_data = sum(1 for label, count in self.label_counts.items() 
                                    if 1 <= label <= 8 and count >= 5)
        action_min_collected = action_labels_with_data >= 3
        ...
        self.complete_btn.setEnabled(game_min_collected and action_min_collected)
    ...
    def on_progress_card_clicked(self, label):
        ...
        if hasattr(self, 'current_label') and self.current_label in self.progress_cards:
            ...
            # 기본 스타일로 복원
            if self.current_label == LABEL_SCISSORS:
                prev_card.setStyleSheet(f"background-color: #EEF2FF; border-radius: 15px;")
            elif self.current_label == LABEL_ROCK:
                prev_card.setStyleSheet(f"background-color: #FFFBEB; border-radius: 15px;")
            elif self.current_label == LABEL_PAPER:
                prev_card.setStyleSheet(f"background-color: #E8F5E9; border-radius: 15px;")
            else:
                prev_card.setStyleSheet(f"background-color: #E6F4FF; border-radius: 15px;")
            
            # 그림자 효과 제거
            prev_card.setGraphicsEffect(None)
        ...
        # 그래픽 효과로 그림자 추가
        shadow = QGraphicsDropShadowEffect(selected_card)
        shadow.setBlurRadius(20)
        shadow.setColor(QColor(PRIMARY_COLOR))
        shadow.setOffset(0, 0)
        selected_card.setGraphicsEffect(shadow)
    ...
    def process_data(self, s1, s2):
        ...
        if (self.game_state == 1 or self.game_state == 2) and isinstance(result, tuple) and result[0] == "movement_completed":
            ...
            if confidence >= self.confidence_threshold:
                # 대기 상태(LABEL_IDLE)이면 플레이어 선택을 변경하지 않음
                if prediction != LABEL_IDLE:
                    self.player_choice = prediction
                    self.update_player_choice()
                    print(f"플레이어 선택 업데이트: {get_label_name(prediction)}")
...
class SettingsWidget(BaseEMGWidget):
    ...
    def init_ui(self):
        ...
        self.rps_round_combo.addItems(["5", "10", "15", "20"])
...
class ScreenControlWidget(BaseEMGWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_window = None  # 현재 활성화된 창 타입
        
        # 동작 설정 딕셔너리 (기본값)
        self.action_settings = {
            "browser": {str(i): "없음" for i in range(1, 14)},  # 1-8, 11-13 지원
        ...
        self.action_settings["browser"]["11"] = "새 탭"  # 가위
        self.action_settings["browser"]["12"] = "탭 닫기"  # 바위
        self.action_settings["browser"]["13"] = "새로고침"  # 보
    ...
    def init_ui(self):
        ...
        info_text = QLabel("EMG 센서를 통해 감지된 동작으로 PC를 제어할 수 있습니다.\n"
                        "각 애플리케이션 타입과 동작에 대한 기능을 설정한 후 '시작' 버튼을 누르세요.\n"
                        "동작 1-8 외에도 가위(11), 바위(12), 보(13) 동작도 제어에 활용할 수 있습니다.")
        info_text.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 18px;")
        info_text.setAlignment(Qt.AlignCenter)
        info_text.setWordWrap(True)
        
        # 현재 상태 표시 - 더 눈에 띄게
        self.status_label = QLabel("대기 중")
        self.status_label.setStyleSheet(f"color: {PRIMARY_COLOR}; font-size: 22px; font-weight: bold;")
        self.status_label.setAlignment(Qt.AlignCenter)
        
        info_layout.addWidget(info_text)
        info_layout.addWidget(self.status_label)
        
        # 앱 선택 카드 추가 (새로운 디자인 요소)
        app_selection_card = NeumorphicCard()
        app_selection_layout = QVBoxLayout(app_selection_card)
        app_selection_layout.setContentsMargins(20, 20, 20, 20)
        
        app_selection_title = QLabel("애플리케이션 선택")
        app_selection_title.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 20px; font-weight: bold;")
        app_selection_title.setAlignment(Qt.AlignCenter)
        
        # 앱 선택 버튼 그리드
        app_button_grid = QGridLayout()
        app_button_grid.setSpacing(15)
        ...
        # 설정 테이블 카드 - 세련된 디자인으로 교체
        settings_card = NeumorphicCard()
        settings_layout = QVBoxLayout(settings_card)
        settings_layout.setContentsMargins(20, 20, 20, 20)
        
        # 설정 테이블 제목
        settings_title = QLabel("동작 설정")
        settings_title.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 20px; font-weight: bold;")
        settings_title.setAlignment(Qt.AlignCenter)
        
        # 스택 위젯으로 각 앱 설정 표시 (한 번에 하나만 표시)
        self.settings_stack = QStackedWidget()
        
        # 브라우저, 유튜브, PPT 설정 페이지 생성
        app_types = [("browser", "인터넷 브라우저"), ("youtube", "유튜브"), ("ppt", "PowerPoint")]
        
        for app_key, app_name in app_types:
            # 앱별 설정 페이지
            app_page = QWidget()
            app_layout = QVBoxLayout(app_page)
            
            # 앱 이름 표시
            app_label = QLabel(f"{app_name} 동작 설정")
            app_label.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 18px; font-weight: bold;")
            ...
            for i in range(1, 9):
                # 라벨 카드 생성
                label_card = NeumorphicCard()
                label_layout = QVBoxLayout(label_card)
                
                # 라벨 이름
                label_name = QLabel(f"동작 {i}")
                label_name.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 16px; font-weight: bold;")
                label_name.setAlignment(Qt.AlignCenter)
                
                # 동작 선택 콤보박스
                combo = QComboBox()
                combo.addItems(self.available_actions[app_key])
                combo.setCurrentText(self.action_settings[app_key].get(str(i), "없음"))
                combo.currentTextChanged.connect(lambda text, a=app_key, l=i: self.update_action_setting(a, l, text))
                ...
                row = (i-1) // 4
                col = (i-1) % 4
                settings_grid.addWidget(label_card, row, col)
            
            # 가위/바위/보 설정 (별도 섹션)
            rps_section = QGroupBox("가위/바위/보 동작 설정")
            rps_section.setStyleSheet(f"""
                QGroupBox {{
                    font-size: 16px;
                    font-weight: bold;
                    color: {TEXT_COLOR};
            ...
            rps_layout = QHBoxLayout(rps_section)
            ...
            for name, key, bg_color in rps_labels:
                # 카드 생성
                rps_card = NeumorphicCard()
                rps_card.setStyleSheet(f"background-color: {bg_color}; border-radius: 15px;")
                rps_card_layout = QVBoxLayout(rps_card)  # 변수명 변경
                
                # 라벨 이름
                rps_label = QLabel(name)
                rps_label.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 16px; font-weight: bold;")
                rps_label.setAlignment(Qt.AlignCenter)
                
                # 동작 선택 콤보박스
                rps_combo = QComboBox()
                rps_combo.addItems(self.available_actions[app_key])
                rps_combo.setCurrentText(self.action_settings[app_key].get(key, "없음"))
                rps_combo.currentTextChanged.connect(lambda text, a=app_key, k=key: self.update_action_setting(a, k, text))
                rps_combo.setStyleSheet(f"""
                    QComboBox {{
                        background-color: {BG_COLOR};
                        border: 1px solid #D1D9E6;
                        border-radius: 8px;
                ...
                rps_card_layout.addWidget(rps_label)
                rps_card_layout.addWidget(rps_combo)
                
                # 레이아웃에 추가 (rps_section의 레이아웃에 추가)
                rps_layout.addWidget(rps_card)
            
            # 페이지에 그리드와 RPS 섹션 추가
            app_layout.addWidget(app_label)
            app_layout.addLayout(settings_grid)
            app_layout.addWidget(rps_section)
        ...
        # 현재 창 상태와 감지된 동작을 가로로 배열
        detection_status = QHBoxLayout()
        
        # 왼쪽: 현재 인식된 창 타입
        window_status = QVBoxLayout()
        
        window_title = QLabel("현재 인식된 창 타입")
        window_title.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 18px; font-weight: bold;")
        window_title.setAlignment(Qt.AlignCenter)
        
        self.window_type_label = QLabel("알 수 없음")
        self.window_type_label.setStyleSheet(f"color: {SECONDARY_COLOR}; font-size: 28px; font-weight: bold;")
        self.window_type_label.setAlignment(Qt.AlignCenter)
        
        window_status.addWidget(window_title)
        window_status.addWidget(self.window_type_label)
        
        # 오른쪽: 현재 감지된 동작
        action_status = QVBoxLayout()
        
        detection_title = QLabel("현재 감지된 동작")
        detection_title.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 18px; font-weight: bold;")
        detection_title.setAlignment(Qt.AlignCenter)
        
        self.detected_label = QLabel("없음")
        self.detected_label.setStyleSheet(f"color: {PRIMARY_COLOR}; font-size: 28px; font-weight: bold;")
        self.detected_label.setAlignment(Qt.AlignCenter)
        
        action_status.addWidget(detection_title)
        action_status.addWidget(self.detected_label)
        
        # 상태 레이아웃에 추가
        detection_status.addLayout(window_status)
        detection_status.addLayout(action_status)
        
        detection_layout.addLayout(detection_status)
    ...
    def _create_app_card(self, name, color, app_key):
        ...
        # 클릭 이벤트 처리
        card.mousePressEvent = lambda event, idx=app_key: self.select_app(app_key)
    ...
    def select_app(self, app_key):
        """앱 타입 선택"""
        # 앱 키에 해당하는 인덱스 찾기
        app_types = ["browser", "youtube", "ppt"]
        if app_key in app_types:
            index = app_types.index(app_key)
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    ...
                    self.action_settings = settings.get("action_settings", self.action_settings)
                    
                # UI 업데이트
                self.update_table_from_settings()
            except:
                pass
    ...
    def update_table_from_settings(self):
        """설정에서 테이블 업데이트"""
        app_types = ["browser", "youtube", "ppt"]
        for row, app_key in enumerate(app_types):
            for col in range(1, 11):
                label_key = str(col)
                action = self.action_settings[app_key][label_key]
                combo = self.settings_table.cellWidget(row, col)
                if combo:
                    combo.setCurrentText(action)
                    
    def update_action_setting(self, app_key, label, text):
        """동작 설정 업데이트"""
        # app_key는 이미 "browser", "youtube", "ppt" 중 하나의 문자열
        # label은 동작 번호 (1-8, 11-13)
        
        label_key = str(label)
        self.action_settings[app_key][label_key] = text
        self.save_settings()
        
        print(f"설정 업데이트: {app_key} - 동작 {label} -> {text}")
    ...
    def detect_current_window(self):
        ...
        try:
            ...
            if "PowerPoint" in window_title or ".ppt" in window_title.lower() or ".pptx" in window_title.lower():
                ...
                self.window_type_label.setStyleSheet(f"color: #FF5722; font-size: 24px; font-weight: bold;")
            elif "YouTube" in window_title:
                self.current_window = "youtube"
                self.window_type_label.setText("유튜브")
                self.window_type_label.setStyleSheet(f"color: #FF0000; font-size: 24px; font-weight: bold;")
                ...
                self.window_type_label.setStyleSheet(f"color: #2196F3; font-size: 24px; font-weight: bold;")
            else:
                self.current_window = None
                self.window_type_label.setText("알 수 없음")
                self.window_type_label.setStyleSheet(f"color: {SECONDARY_COLOR}; font-size: 24px; font-weight: bold;")
        except:
            self.current_window = None
            self.window_type_label.setText("감지 실패")
            self.window_type_label.setStyleSheet(f"color: {DANGER_COLOR}; font-size: 24px; font-weight: bold;")
...
class EMGGameApplication(QMainWindow):
    ...
    def resizeEvent(self, event):
        """창 크기 변경 이벤트 처리"""
        # 창 사이즈가 일정 값 이상일 때만 스케일링 적용
        if self.width() >= 500 and self.height() >= 400:
            # 기본 사이즈 대비 변화율 계산 (가로, 세로 중 작은 비율 적용)
            width_scale = self.width() / 800
            height_scale = self.height() / 600
            scale_factor = min(width_scale, height_scale)
        
            # 스케일 팩터가 일정 범위 내에 있고, 이전과 충분히 달라졌을 때만 적용
            if 0.5 <= scale_factor <= 2.0 and abs(scale_factor - self.last_scale_factor) > 0.05:
    ...
    def adjust_window_size(self, width, height):
        ...
        # 현재 크기가 이미 요청된 크기보다 크면 크기를 변경하지 않음
        if current_size.width() >= width and current_size.height() >= height:
            return
            
        # 창 크기 조절 애니메이션 생성
        self.animation = QPropertyAnimation(self, b"size")
        self.animation.setDuration(300)  # 300ms 동안 애니메이션
        self.animation.setStartValue(current_size)
        self.animation.setEndValue(QSize(width, height))
