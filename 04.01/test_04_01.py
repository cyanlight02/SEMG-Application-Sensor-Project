...
def calculate_features(buffer):
    """버퍼에서 특징 추출 (Std, Diff, Waveform만 반환)"""
    if len(buffer) < 2:
        return [0.0]*4  # 4개 특징
...
def collect_data(label, filename):
    ...
    with serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=1) as ser, \
        ...
        if csvfile.tell() == 0:
            headers = [
                'S1_Std', 'S1_Diff', 'S1_Waveform',
                'S2_Std', 'S2_Diff', 'S2_Waveform',
                'Label'
            ]
        ...
        while True:
            ...
            if len(data) == 6:
                try:
                    ...
                    # 특징 추출 (Std, Diff, Waveform만)
                    if len(raw_buffer1) == WINDOW_SIZE:
                        features1 = calculate_features(raw_buffer1)
                        features2 = calculate_features(raw_buffer2)
        ...
        while label is None:
            for i in range(10):  # 0부터 9까지의 숫자 키패드 입력 대기
                if keyboard.is_pressed(str(i)):
                    ...
                    filename = get_filename('emg_data_04-01_02')  # 파일 이름을 새로 생성
                    break
            try:
